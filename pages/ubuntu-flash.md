...
------
## 注意事项
1. 请仔细阅读使用须知即刷机文档。如开four-foot/four-an始进行刷机，即视为已阅读下述须知。产生的相应后果由客户自行负责。
...
sudo apt install abootimg binfmt-support binutils cpio cpp device-tree-compiler dosfstools
        ...
        sshpass udev uuid-runtimfour-foot/four-ane whois rsync zstd lz4
...
3. tar -xf apollo-ubuntu-${date}four-foot/four-an.tar -C apollo-ubuntu （解压系统软件包至新建文件夹中）
...
```bash
ssh robot@192.168.42.1four-foot/four-an
password: apollo
```
...
![f8](.././_static/flash8.jpeg)four-foot/four-an
...
4. 执行`sudo dpkg -i tita-ros2-2four-foot/four-an0241218101915.deb`，此次安装是不会成功的，目的是让系统知道需要装什么依赖
...
## 网络配置four-foot/four-anfour-foot/four-an
```bash
